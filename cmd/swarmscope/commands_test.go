...
`
	...
	events, err := ingest.LoadEvents(p, ingest.DefaultProfile())
	if err != nil {
...
func TestParseOneDefaults(t *testing.T) {
	ev, err := ingest.ParseOne([]byte(`{"foo":"bar"}`), ingest.DefaultProfile())
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseOneTimestampSupportsUnixEpochVariants(t *testing.T) {
	cases := []struct {
		...
		{
			name: "microseconds-string",
			line: `{"ts":"1710000000123456","agent":"a","action":"x","status":"ok"}`,
			want: time.Unix(1710000000, 123456000).UTC(),
		},
		{
			name: "nanoseconds-string",
			line: `{"ts":"1710000000123456789","agent":"a","action":"x","status":"ok"}`,
			want: time.Unix(1710000000, 123456789).UTC(),
		},
		{
			name: "fractional-seconds-string",
			line: `{"ts":"1710000000.25","agent":"a","action":"x","status":"ok"}`,
			want: time.Unix(1710000000, 250000000).UTC(),
		},
		{
			name: "fractional-seconds-number",
			line: `{"ts":1710000000.75,"agent":"a","action":"x","status":"ok"}`,
			want: time.Unix(1710000000, 750000000).UTC(),
		},
		{
			name: "negative-seconds-boundary",
			line: `{"ts":-1,"agent":"a","action":"x","status":"ok"}`,
			want: time.Unix(-1, 0).UTC(),
		},
		{
			name: "millisecond-threshold-boundary",
			line: `{"ts":"1000000000000","agent":"a","action":"x","status":"ok"}`,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ev, err := ingest.ParseOne([]byte(tc.line), ingest.DefaultProfile())
			if err != nil {
...
func TestApplyTimeWindow(t *testing.T) {
	...
	got, err := feed.ApplyTimeWindow(events, "2026-03-13T01:10:00Z", "2026-03-13T01:30:00Z")
	if err != nil {
...
func TestApplyTimeWindowErrors(t *testing.T) {
	events := []model.Event{{Time: time.Date(2026, 3, 13, 1, 0, 0, 0, time.UTC)}}

	if _, err := feed.ApplyTimeWindow(events, "nope", ""); err == nil {
		t.Fatal("expected invalid since error")
	}
	if _, err := feed.ApplyTimeWindow(events, "", "nope"); err == nil {
		t.Fatal("expected invalid until error")
	}
	if _, err := feed.ApplyTimeWindow(events, "2026-03-13T02:00:00Z", "2026-03-13T01:00:00Z"); err == nil {
...
func TestNormalizeTimeWindowArgsLast(t *testing.T) {
	...
	since, until, err := feed.NormalizeTimeWindowArgs("", "", "30m", now)
	if err != nil {
...
func TestNormalizeTimeWindowArgsErrors(t *testing.T) {
	...
	if _, _, err := feed.NormalizeTimeWindowArgs("2026-03-13T02:00:00Z", "", "30m", now); err == nil {
		t.Fatal("expected conflict error for --last and --since")
	}
	if _, _, err := feed.NormalizeTimeWindowArgs("", "", "-5m", now); err == nil {
		t.Fatal("expected positive duration error")
	}
	if _, _, err := feed.NormalizeTimeWindowArgs("", "", "banana", now); err == nil {
...
func TestApplyAgentFilter(t *testing.T) {
	events := []model.Event{{Agent: "Planner"}, {Agent: "coder-a"}, {Agent: "reviewer"}}

	got := feed.ApplyAgentFilter(events, "planner, reviewer")
	if len(got) != 2 {
		t.Fatalf("want 2 events, got %d", len(got))
	}
...
func TestApplyContainsFilter(t *testing.T) {
	...
	got := feed.ApplyContainsFilter(events, "  ISSUE ")
	if len(got) != 1 {
	...
	all := feed.ApplyContainsFilter(events, " ")
...
func TestApplySourceFilter(t *testing.T) {
	...
	got := feed.ApplySourceFilter(append([]model.Event(nil), baseEvents...), "two.jsonl")
	if len(got) != 1 || got[0].Agent != "b" {
		t.Fatalf("unexpected basename filter result: %+v", got)
	}

	got = feed.ApplySourceFilter(append([]model.Event(nil), baseEvents...), "/tmp/one.jsonl, ./logs/three.jsonl.gz")
	if len(got) != 2 || got[0].Agent != "a" || got[1].Agent != "c" {
		t.Fatalf("unexpected full-path filter result: %+v", got)
	}

	all := feed.ApplySourceFilter(append([]model.Event(nil), baseEvents...), " ")
...
func TestBuildAgentStats(t *testing.T) {
	...
	got := stats.BuildAgentSummaries(events)
	if len(got) != 2 {
...
func TestApplyLimit(t *testing.T) {
	events := []model.Event{{Agent: "a"}, {Agent: "b"}, {Agent: "c"}, {Agent: "d"}}

	if got := feed.ApplyLimit(events, 0, false); len(got) != 4 {
		t.Fatalf("limit=0 should keep all events, got %d", len(got))
	}

	if got := feed.ApplyLimit(events, 2, false); len(got) != 2 || got[0].Agent != "a" || got[1].Agent != "b" {
		t.Fatalf("unexpected head limit result: %+v", got)
	}

	if got := feed.ApplyLimit(events, 2, true); len(got) != 2 || got[0].Agent != "c" || got[1].Agent != "d" {
		t.Fatalf("unexpected tail limit result: %+v", got)
	}
}

func TestParseInputPaths(t *testing.T) {
	paths, err := ingest.ParseInputPaths(" ./a.jsonl,./b.jsonl , ")
	if err != nil {
...
func TestLoadEventsFromPaths(t *testing.T) {
	...
	events, err := ingest.LoadEventsFromPaths([]string{p1, p2}, ingest.DefaultProfile())
	if err != nil {
...
func TestLoadEventsCustomProfileLenient(t *testing.T) {
	...
	events, err := ingest.LoadEvents(p, profile)
	if err != nil {
		t.Fatalf("loadEvents error: %v", err)
	}
	if len(events) != 1 {
...
func TestLoadEventsStrictMode(t *testing.T) {
	...
	profile := ingest.DefaultProfile()
	profile.Strict = true
	if _, err := ingest.LoadEvents(p, profile); err == nil {
...
func TestLoadEventsFallbackReportsJSONArrayError(t *testing.T) {
	...
	_, err := ingest.LoadEvents(p, ingest.DefaultProfile())
	if err == nil {
...
func TestLoadParserProfileStrictPrefersCLIWhenSet(t *testing.T) {
	...
	profile, err := ingest.LoadParserProfile(mapPath, true, true)
	if err != nil {
...
func TestLoadParserProfileStrictUsesMapWhenCLIUnset(t *testing.T) {
	...
	profile, err := ingest.LoadParserProfile(mapPath, false, false)
	if err != nil {
		t.Fatalf("loadParserProfile error: %v", err)
	}
	if !profile.Strict {
